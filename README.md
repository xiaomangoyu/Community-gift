...
| 文字策略 | `text_plan` | 判断文字是 `one_shot_text` 还是 `post_overlay_text` |
| Prompt 计划 | `prompt_plan` | 固定摄影、产品结构、材质、保留元素、禁用元素和重试焦点 |

长英文、多语言、特殊标点名字会自动切到 `post_overlay_text`：生图阶段只生成空白中央发光铭牌 / 核心，后续再程序化覆盖精确文字。短拉丁文字才继续尝试 one-shot。
...
默认编排方式是 template-first：不让 LLM 从主播数据重新设计 prompt，而是使用稳定参考 prompt 骨架，只填入少量主播变量。旧的 LLM/planner 编排只能通过 `--legacy-llm-design` 显式开启。
...
- `one_shot_text` 模式下，唯一文字必须放在灯头中央发光核心或中央嵌入铭牌里，水平、居中、清晰。
- `post_overlay_text` 模式下，图像模型不得生成任何文字，只生成空白中央铭牌 / 发光核心。
