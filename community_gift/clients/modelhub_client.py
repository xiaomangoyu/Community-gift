...
6. materials: 只选两种主材/辅材,必须是产品材质。默认主材优先从软搪胶、亲肤硅胶、短绒植绒、
   毛绒/布艺包覆、柔雾珐琅、泡棉、陶土、木质、柔软果冻树脂里选。玻璃、晶体、金属只能作为
   小面积点缀或极薄连接边,不要作为大面积灯头/握柄主材。
...
class ModelHubGiftClient:
    ...
    def analyze_host_visual_brief(self, host: HostInput) -> HostVisionBrief:
        ...
        signals_summary = {
            ...
            "palette_direction": raw.get("palette_direction", []),
            "material_direction": raw.get("material_direction", []),
            "mood_coverage": raw.get("mood_coverage", []),
            "form_exploration": raw.get("form_exploration", []),
        ...
        instruction = (
            ...
            "  - materials.main 例:哑光软搪胶 / 亲肤硅胶 / 短绒植绒 / 布艺包覆 / 柔软果冻树脂\n"
            ...
            "  - lighting 例:清晰主光、柔和边缘透光、软硬高光对比、局部内发光\n"
            "  - mood.phrase 例:甜美、治愈、俏皮、亲切、软萌、带潮玩感\n"
            "  - handle.main_material 例:珠光软搪胶 / 亲肤硅胶 / 短绒布艺包覆 / 柔雾珐琅\n"
            ...
            "柔和过渡至灯头的曲面收口/ 一圈半透软树脂环 + 微型主题切片(不要粗金属箍环,不要橡胶圈)\n"
            ...
            "10. **材质必须有显著差异性,并且默认软触感主导**。主导家族优先从 A-E 选择,"
            "辅助材质来自另一个家族形成对比。不允许两项都属同一家族:\n"
            "   A. 软胶/硅胶家族 — 哑光软搪胶、亲肤硅胶、软乙烯、软糖弹性质感\n"
            "   B. 毛绒/织物家族 — 短绒植绒、羊毛、丝绒、布艺包覆、毛毡、刺绣\n"
            "   C. 柔雾涂层家族 — 柔雾珐琅、哑光橡胶喷漆、磨砂保护漆、丝绒触感涂层\n"
            "   D. 手作陶木家族 — 哑光陶、釉面陶、漆木、藤编、软木纹理\n"
            "   E. 柔软树脂家族 — 柔软果冻树脂、珠光软树脂、软发光树脂\n"
            "   F. 玻璃晶体家族 — 切面水晶、磨砂玻璃、吹制气泡玻璃、彩色玻璃(只能小面积)\n"
            "   G. 金属家族 — 拉丝铝、阳极氧化枪灰、镜面铬、做旧黄铜、玫瑰金(只能极薄饰边)\n"
            "   H. 自然纹理家族 — 贝母、骨刻、沙岩、矿物结晶(只能局部点缀)\n"
            "**严禁默认到 '吹制气泡玻璃 + 金属/黑铝'、'枪灰金属 + 烟熏亚克力' 或 '珠光树脂 + 香槟金' 这些硬质套路**。\n"
            "选家族时:\n"
            "   - 优先从 material_direction / avatar 看到的软材质(布料/毛发/毛绒/皮革/陶/木/硅胶感)出发\n"
            "   - 其次从 characterization 中的内容形态(只有真正的 KO/PUBG/Free Fire/PK 才算 military)\n"
            "   - 可爱/宠物/食品/贴纸/治愈/社区礼物默认使用软搪胶、硅胶、植绒、布艺、泡棉或柔雾珐琅\n"
            "   - 文化暗示也要软化:异域/沙漠 → 柔雾珐琅 + 沙岩点缀;海岛 → 柔软果冻树脂 + 贝母点缀;"
            "都市夜店 → 哑光喷漆 + 小面积透明树脂;市井日常 → 陶 + 木\n"
            "   - 镜子/黑银/冷峻主题也不要写真实镜面、亮黑钢琴漆或强白反射条;"
            "转译为雾银软框、柔雾珐琅内芯、烟灰软胶、低亮宽柔高光\n"
            "主辅之间必须有视觉对比:光泽 vs 磨砂、硬点缀 vs 软主材、冷 vs 暖、反射 vs 吸光。\n"
            "如果确实需要玻璃/晶体/金属,只能写在 supporting 或 handle.connector_detail,并加'小面积/极薄/局部'限定。\n"
